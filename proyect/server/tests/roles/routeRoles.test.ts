...
describe("Roles Routes", () => {
  ...
  describe("GET /roles", () => {
    it("debe devolver un array de roles cuando la base de datos tiene datos", async () => {
      ...
      mockRolesModel.getAll.mockResolvedValue(mockRoles);
    ...
    it("debe devolver un array vacío cuando la base de datos no tiene datos", async () => {
      // Arrange
      mockRolesModel.getAll.mockResolvedValue([]);
    ...
    it("debe devolver un error 500 si falla la obtención de datos", async () => {
      // Arrange
      mockRolesModel.getAll.mockRejectedValue(
  ...
  describe("GET /roles/:idRol", () => {
    it("debe devolver un rol cuando el idRol existe", async () => {
      // Arrange
      const mockRol: Rol = {
        idRol: 1,
        rol: "Admin",
      };
      mockRolesModel.getById.mockResolvedValue(mockRol);
    ...
    it("debe devolver un error 404 cuando el idRol no existe", async () => {
      // Arrange
      mockRolesModel.getById.mockResolvedValue(null);
    ...
    it("debe devolver un error 500 si ocurre un fallo en la base de datos", async () => {
      // Arrange
      mockRolesModel.getById.mockRejectedValue(
        new Error("Error de base de datos")
      );
    ...
    it("debe manejar correctamente un idRol inválido (NaN)", async () => {
      // Act
  ...
  describe("POST /roles", () => {
    it("debe crear un rol cuando los datos son válidos", async () => {
      ...
      mockRolesModel.create.mockResolvedValue({ idRol: 1, ...newRol });
      ...
      expect(response.body).toEqual({ idRol: 1, ...newRol });
    });

    it("debe devolver 400 si los datos son inválidos", async () => {
      // Arrange
      const invalidRol = { rol: "" }; // Nombre vacío
      mockRolesModel.create.mockResolvedValue(null);

      // Act
      const response = await request(app).post("/roles").send(invalidRol);
    ...
    it("debe devolver 400 si el rol ya existe", async () => {
      ...
      mockRolesModel.create.mockRejectedValue(new Error("already exists"));
    ...
    it("debe devolver 500 si hay un error interno", async () => {
      // Arrange
      const newRol = { rol: "Usuario" };
      mockRolesModel.create.mockRejectedValue(new Error("Error inesperado"));
  ...
  describe.only("PUT /roles/:idRol", () => {
    it("debe actualizar un rol cuando los datos son válidos", async () => {
      // Arrange
      const idRol = 1;
      const updatedData = { rol: "SuperAdmin" };
      mockRolesModel.update.mockResolvedValue({ idRol, ...updatedData });
      ...
      // Assert
      expect(response.status).toBe(200);
      expect(response.body).toEqual({ idRol, ...updatedData });
      expect(mockRolesModel.update).toHaveBeenCalledWith(idRol, updatedData);
    });

    it("should return 400 if validation fails", async () => {
      // Arrange
      const idRol = 1;
      const invalidData = { rol: "" }; // Empty role name
      mockRolesModel.update.mockResolvedValue(null);
      ...
      expect(response.status).toBe(400);
      expect(response.body).toHaveProperty("error");
      expect(mockRolesModel.update).not.toHaveBeenCalled();
    });
    ...
    it("debe devolver 500 si ocurre un error en el servidor", async () => {
      ...
      const idRol = 1;
      const updatedData = { rol: "SuperAdmin" };
      mockRolesModel.update.mockRejectedValue(
        new Error("Error de base de datos")
      );
    ...
    it("debe devolver 400 si el idRol es inválido", async () => {
      // Act
      const response = await request(app)
        .put("/roles/abc") // idRol inválido
        .send({ rol: "NuevoRol" });
  ...
  describe("DELETE /roles/:idRol", () => {
    it("debería eliminar un rol existente y devolver un estado 200", async () => {
      // Simulamos una eliminación exitosa
      mockRolesModel.delete.mockResolvedValue(true);

      const response = await request(app).delete("/roles/1");

      expect(mockRolesModel.delete).toHaveBeenCalledWith(1); // Se llama con el ID correcto
    ...
    it("debería devolver un estado 404 si el rol no existe", async () => {
      ...
      mockRolesModel.delete.mockResolvedValue(false);

      const response = await request(app).delete("/roles/999");

      expect(mockRolesModel.delete).toHaveBeenCalledWith(999); // Se llama con el ID incorrecto
    ...
    it("debería devolver un estado 500 si ocurre un error interno", async () => {
      ...
      mockRolesModel.delete.mockRejectedValue(
        new Error("Error de base de datos")
      );

      const response = await request(app).delete("/roles/2");

      expect(mockRolesModel.delete).toHaveBeenCalledWith(2); // Se llama con el ID correcto
